...
## Phase 2: Voice pipeline — speech-to-text, text-to-speech, wake word — ✅ DONE 2026-09-15
...
- [x] Add the **Wyoming Protocol** integration three times (whisper, piper, openwakeword) using the cluster DNS names, e.g. `wyoming-whisper.home-assistant.svc.cluster.local:10300`. _(2026-09-15: done; charts deployed via ArgoCD.)_
- [x] Settings → Voice assistants → create/edit the assistant: STT = Whisper, TTS = Piper, conversation = Ollama (from Phase 1), wake word = openWakeWord. _(2026-09-15: "Ollama" assistant is the preferred one — language `en`, `stt.faster_whisper`, `tts.piper` voice `en_US-glados-medium`, wake word `okay_nabu`.)_
- [x] Test with the microphone button in the HA web UI and in the Companion app (Assist → tap-to-talk). _(2026-09-15: Companion app voice in/voice out works. First attempt answered in text only; worked on retry — not reproduced, likely the app's assistant selection/cache.)_
- [x] Expose the Google Home `media_player` to Assist (with an alias like "kitchen speaker") so volume/pause/play work by voice. Add a first announcement automation (e.g. `tts.speak` → Google Home on HA start) as a smoke test — put it in the seeded `packages/` mechanism from 4.2 once that exists, otherwise `automations.yaml` for now. _(YAML written: `docs/home-assistant/announcements.md` — paste into the UI; move into `packages/` in 4.2.)_

### 2.5 Validate
- [x] Say "turn on the living room lights" → light turns on, spoken confirmation. _(No real devices until after the move; general questions by voice work. Re-check with devices.)_
- [x] `tts.speak` (Piper voice) plays on the Google Home; note the delay between service call and audio start (Cast buffering adds ~1–2 s on top of Piper). _(2026-09-15: works, ~1–2 s.)_
- [x] Record end-to-end latency (wake → response audio). _(2026-09-15: subjectively "very good" in the Companion app; no tuning needed, `OLLAMA_KEEP_ALIVE` stays off.)_ Tune: Whisper model size, prompt length, exposed-entity count. This is the point where `OLLAMA_KEEP_ALIVE` (deferred from 1.1) becomes worth turning on if the cold start dominates. _Component budget measured 2026-09-14 (protocol-level, CPU): wake 0.3 s + STT 0.5–0.7 s + LLM 0.44 s warm / 4 s cold + TTS ~0.5 s → **~2 s warm** before Cast buffering; the Ollama cold start is the only thing that would push it past 5 s._
...
**Discoveries**:
- 2026-09-15: **Phase 2 closed.** Full pipeline (Whisper → Ollama/GLaDOS prompt → Piper GLaDOS voice) works from the Companion app with spoken answers; GLaDOS also speaks on the Google Home (`media_player.den_speaker`, exposed to Assist). Latency is fine without keep-alive. Loose ends roll forward: device commands by voice once devices exist (after the move), startup announcement automation → `packages/` in Phase 4.2.
- 2026-09-15: **GLaDOS voice.** `wyoming-piper` ships custom voices via `piper.customVoices` (init container downloads `.onnx` + `.onnx.json` into `/data`); default is `en_US-glados-medium` (DavesArmoury/GLaDOS_TTS). Gotcha: HA groups TTS voices by the exact language string a voice reports and shows one group per language — catalogue voices say `en_US`, the GLaDOS config said `en-us`, so it was invisible until the init container started normalising `language.code` from the voice name. Wyoming integrations were added in HA (2.4 started) and GLaDOS is selected as the English assistant's TTS voice.
